...
import { gost256A, gost512C } from "./index.js";
...
describe("[SIGN] Point conversion (256 bit)", () => {
    ...
    test("(u,v) -> (x,y)", () => {
        const result = gost256A.utils.uv2xy(point);
        expect(result.x).toStrictEqual(point2.x);
        expect(result.y).toStrictEqual(point2.y);
    });

    test("(x,y) -> (u,v)", () => {
        const result = gost256A.utils.xy2uv(point2);
...
describe("[SIGN] Point conversion (512 bit)", () => {
    ...
    test("(u,v) -> (x,y)", () => {
        const result = gost512C.utils.uv2xy(point);
        expect(result.x).toStrictEqual(point2.x);
        expect(result.y).toStrictEqual(point2.y);
    });

    test("(x,y) -> (u,v)", () => {
        const result = gost512C.utils.xy2uv(point2);
        expect(result.x).toStrictEqual(point.x);
        expect(result.y).toStrictEqual(point.y);
    });
});